...
async function main() {
  try {
    ...
    console.log('🚀 启动完成，正在启动 UI...\n');

    // Dynamically import React and Ink modules to avoid ES module conflicts
    const React = await import('react');
    const ink = await import('ink' as any);
    const ChatInterfaceModule = await import('./components/ChatInterface');
    
    const { render } = ink;
    const ChatInterface = (ChatInterfaceModule as any).default || (ChatInterfaceModule as any).ChatInterface;

    // Start the CLI interface using dynamic imports
    render(React.createElement(ChatInterface, { engine: conversationEngine }));
